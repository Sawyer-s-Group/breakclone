...
getHitCN <- function(sample1, sample2, pair, position = NULL, cnType = c("alleleSpecific", "VCF"), maxgap) {
  ...
  if (cnType == "alleleSpecific") {
    ...
    sample1_lists <- sapply(presentStates, function(x) {
      c(list(sample1[x, on = "SVType"]))
    })
    sample2_lists <- sapply(presentStates, function(x) {
      c(list(sample2[x, on = "SVType"]))
    })
  }

  # tryCatch creates an empty GRanges object if the list is empty - would error out otherwise
  sample1_granges <- lapply(sample1_lists, function(x) {
    tryCatch(
      {
  ...
  sample2_granges <- lapply(sample2_lists, function(x) {
    tryCatch(
      {
        makeGRangesFromDataFrame(x)
      },
  ...
  hits <- mapply(function(x, y) {
    x[queryHits(suppressWarnings(findOverlaps(x, y, type = position, maxgap = maxgap)))]
  }, sample1_granges, sample2_granges)
  return(hits)
}
...
getScoreCN <- function(segmentTable, pair, populationBreakpoints, cnType, maxgap) {
  ...
  score_from_hits_start <- sum(unlist(lapply(hits_start, function(x) {
    1 - countOverlaps(query = x, subject = populationBreakpoints$Starts, type = "start", maxgap = maxgap) / length(unique(segmentTable$SampleID))
  })))
  if (score_from_hits_start < 0) {
    warning("Hit next probe! Consider lowering the maxgap")
    score_from_hits_start <- 0
  }
  score_from_hits_end <- sum(unlist(lapply(hits_end, function(x) {
    1 - countOverlaps(query = x, subject = populationBreakpoints$Ends, type = "end", maxgap = maxgap) / length(unique(segmentTable$SampleID))
  })))
...
calculateRelatednessCn <- function(segmentTable, pairs, reference = NULL, cnType = c("alleleSpecific", "VCF"), excludeChromosomes = "Y", maxgap = NULL) {
  ...
  pair_scores <- apply(pairs, 1, function(x) {
    getScoreCN(segmentTable, as.character(x), populationBreakpoints, cnType, maxgap)
  })
